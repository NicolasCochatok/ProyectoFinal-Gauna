...
function ItemDetail({ item }) {
  ...
  const qtyEnCarrito = getItemQty(item.id)
  const handleAdd    = (qty) => {
    setJustAdded(qty)
    addItem(
      { id: item.id, title, price: typeof price === 'number' ? price : 0, stock, image: imgSrc },
      qty
    )
  }

  const totalAgregado = qtyEnCarrito + justAdded
  const mostrarBotones = totalAgregado > 0
  ...
  return (
    ...
    >
      ...
      <p>{description}</p>
      <p>
        <strong>Precio:</strong> {typeof price === 'number' ? `$${price}` : price}
      </p>
      <p>
